...
The v1 covergroup schema is being designed now. If you maintain a production voice agent and have scenarios you wish were tracked as first-class verification targets — refusal-then-retry, ambiguous-extract, mid-call interrupt with go-back — post them on the RFC: [voicetest#48](https://github.com/voicetestdev/voicetest/issues/48). Two or three covergroups from a real graph will shape the grammar more than weeks of internal speculation.
{% endraw %}
